          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            >
              <Card>
                ...
                <p style={{ color: "#64748b", marginTop: 4 }}>Set the portfolio context and test design in real time with the agency.</p>
                ...
                <div style={{ display: "grid", gap: 18 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <Field label="Benchmark mode">
                      <SelectInput value={state.benchmarkMode} onChange={(v) => update("benchmarkMode", v)} />
                    </Field>
                    <Field label="Accounts in portfolio">
                      <NumberInput value={state.accountCount} onChange={(v) => update("accountCount", v)} />
                    </Field>
                  </div>

                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <Field label="Current monthly spend">
                      <NumberInput value={state.monthlySpend} onChange={(v) => update("monthlySpend", v)} />
                    </Field>
                    <Field label="Current monthly conversions">
                      <NumberInput value={state.monthlyConversions} onChange={(v) => update("monthlyConversions", v)} />
                    </Field>
                    <Field label="Prior monthly spend">
                      <NumberInput value={state.priorMonthlySpend} onChange={(v) => update("priorMonthlySpend", v)} />
                    </Field>
                    <Field label="Prior monthly conversions">
                      <NumberInput value={state.priorMonthlyConversions} onChange={(v) => update("priorMonthlyConversions", v)} />
                    </Field>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <Field label="Current win rate (%)">
                      <NumberInput value={state.winRate} onChange={(v) => update("winRate", v)} />
                    </Field>
                    <Field label="Prior win rate (%)">
                      <NumberInput value={state.priorWinRate} onChange={(v) => update("priorWinRate", v)} />
                    </Field>
                    <Field label="Current avg sales cycle (days)">
                      <NumberInput value={state.avgSalesCycleDays} onChange={(v) => update("avgSalesCycleDays", v)} />
                    </Field>
                    <Field label="Prior avg sales cycle (days)">
                      <NumberInput value={state.priorSalesCycleDays} onChange={(v) => update("priorSalesCycleDays", v)} />
                    </Field>
                    <Field label="Deals above stage-aging threshold (%)">
                      <NumberInput value={state.stageAgingOverThreshold} onChange={(v) => update("stageAgingOverThreshold", v)} />
                    </Field>
                    <Field label="Readout window (weeks)">
                      <NumberInput value={state.targetReadoutWeeks} onChange={(v) => update("targetReadoutWeeks", v)} />
                    </Field>
                  </div>

                  <Field label="Test reallocation (%)">
                    <NumberInput value={state.testReallocation} onChange={(v) => update("testReallocation", v)} />
                  </Field>
                </div>
              </Card>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                      ...
                      <motion.div key={card.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }}>
                        <Card>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                            <div style={{ background: "#f1f5f9", borderRadius: 14, padding: 8 }}>
                              <Icon size={16} />
                            </div>
                ...
                <Card>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                    <div>
                      ...
                      <p style={{ marginTop: 6, color: "#64748b" }}>Use this live to reframe the conversation and propose a next step.</p>
                    </div>
                  ...
                  >
                    <div>
                      ...
                      >
                        <div style={{ background: "#f1f5f9", borderRadius: 18, padding: 16 }}>
                          <div style={{ fontSize: 12, textTransform: "uppercase", color: "#64748b" }}>Recommended shift</div>
                          <div style={{ marginTop: 8, fontSize: 24, fontWeight: 700 }}>{analysis.suggestedShift}%</div>
                          <div style={{ marginTop: 4, fontSize: 14, color: "#475569" }}>From saturated capture into discovery</div>
                        </div>
                        <div style={{ background: "#f1f5f9", borderRadius: 18, padding: 16 }}>
                          <div style={{ fontSize: 12, textTransform: "uppercase", color: "#64748b" }}>Modeled CAC improvement</div>
                          <div style={{ marginTop: 8, fontSize: 24, fontWeight: 700 }}>{formatPct(analysis.modeledCACImprovementPct)}</div>
                          <div style={{ marginTop: 4, fontSize: 14, color: "#475569" }}>Hold-flat spend test</div>
                        </div>
                        <div style={{ background: "#f1f5f9", borderRadius: 18, padding: 16 }}>
                          <div style={{ fontSize: 12, textTransform: "uppercase", color: "#64748b" }}>Incremental conversions</div>
                          <div style={{ marginTop: 8, fontSize: 24, fontWeight: 700 }}>{Math.round(analysis.incrementalConversions)}</div>
                          <div style={{ marginTop: 4, fontSize: 14, color: "#475569" }}>Modeled from new blended CAC</div>
                        </div>
                      </div>
                      ...
                      <div style={{ marginTop: 20, border: "1px solid #e2e8f0", borderRadius: 18, padding: 16 }}>
                        ...
                        <p style={{ margin: 0, fontSize: 14, lineHeight: 1.8, color: "#334155" }}>
                          Right now, the issue looks less like pure channel performance and more like a portfolio-efficiency problem. We would run a controlled {analysis.suggestedShift}% reallocation from capture into discovery over {state.targetReadoutWeeks} weeks, hold spend flat, and inspect blended CAC, total output, and incremental lift before scaling.
                        </p>
                      </div>
                    </div>
                    ...
                    <div style={{ background: "#f1f5f9", borderRadius: 24, padding: 20 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 14, fontSize: 14, color: "#334155" }}>
                        {[
                          "Choose one account or cohort where CAC is drifting.",
                          "Run a controlled reallocation with spend held flat.",
                          "Measure blended CAC, total output, and incremental lift.",
                        ].map((t) => (
                      ...
                      <div style={{ marginTop: 20, background: "#fff", borderRadius: 18, padding: 16 }}>
                        ...
                        <div style={{ marginTop: 10, fontSize: 14, lineHeight: 1.7, color: "#334155" }}>
                          Once the client engages on the test or shows deeper interest, move to the Revenue Fragility Diagnostic to expand the relationship beyond channel mix.
                        </div>
                        <div style={{ marginTop: 16 }}>
                          <Button onClick={() => setScreen("fragility")}>Continue to structural diagnosis</Button>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                >
                  ...
                  <Card>
                    ...
                    <div style={{ width: "100%", height: 320 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={analysis.mixData}>
                          <CartesianGrid strokeDasharray="3 3" />
                          <XAxis dataKey="name" />
                          <YAxis />
                          <Tooltip />
                          <Bar dataKey="capture" stackId="a" />
                          <Bar dataKey="discovery" stackId="a" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </Card>
            ...
            >
              <Card>
                ...
                <div style={{ display: "grid", gap: 18 }}>
                  <Field label="Top 3 revenue concentration (%)">
                    <NumberInput value={state.top3RevenueConcentration} onChange={(v) => update("top3RevenueConcentration", v)} />
                  </Field>
                  <Field label="Founder-dependent late-stage deals (%)">
                    <NumberInput value={state.founderDependentDeals} onChange={(v) => update("founderDependentDeals", v)} />
                  </Field>
                  <Field label="Forecast accuracy (%)">
                    <NumberInput value={state.forecastAccuracy} onChange={(v) => update("forecastAccuracy", v)} />
                  </Field>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <Field label="Current gross margin (%)">
                      <NumberInput value={state.grossMargin} onChange={(v) => update("grossMargin", v)} />
                    </Field>
                    <Field label="Prior gross margin (%)">
                      <NumberInput value={state.priorGrossMargin} onChange={(v) => update("priorGrossMargin", v)} />
                    </Field>
                  </div>
                  <Field label="Manager inspection rate (%)">
                    <NumberInput value={state.managerInspectionRate} onChange={(v) => update("managerInspectionRate", v)} />
                  </Field>
                  <Field label="Revenue governance score (%)">
                    <NumberInput value={state.revenueGovernanceScore} onChange={(v) => update("revenueGovernanceScore", v)} />
                  </Field>
                </div>
              </Card>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                ...
                <Card>
                  ...
                  >
                    <div>
                      ...
                      >
                        {analysis.fragilityPillars.map((pillar) => (
                          <div key={pillar.name} style={{ background: "#f1f5f9", borderRadius: 18, padding: 16 }}>
                            <div style={{ fontSize: 12, textTransform: "uppercase", color: "#64748b" }}>{pillar.name}</div>
                            <div style={{ marginTop: 8, fontSize: 24, fontWeight: 700 }}>{pillar.score}/100</div>
                            <div style={{ marginTop: 4, fontSize: 14, color: "#475569" }}>{scoreLabel(pillar.score)}</div>
                          </div>
                        ))}
                      </div>

                      <div style={{ marginTop: 20, border: "1px solid #e2e8f0", borderRadius: 18, padding: 16 }}>
                        <div style={{ fontWeight: 600, marginBottom: 8 }}>Conversation bridge</div>
                        <p style={{ margin: 0, fontSize: 14, lineHeight: 1.8, color: "#334155" }}>
                          Now that we’ve identified a portfolio efficiency opportunity, this diagnostic helps us assess whether there are broader structural issues—forecast reliability, concentration risk, dependency, and margin durability—that may limit growth as the business scales.
                        </p>
                      </div>
                    </div>
                    ...
                    <div style={{ background: "#f1f5f9", borderRadius: 24, padding: 20 }}>
                      ...
                      <div style={{ marginTop: 20, background: "#fff", borderRadius: 18, padding: 16 }}>
                        <div style={{ fontSize: 12, textTransform: "uppercase", color: "#64748b" }}>Recommended use case</div>
                        <div style={{ marginTop: 10, fontSize: 14, lineHeight: 1.7, color: "#334155" }}>
                          Use with agencies that have already engaged on testing, shown strategic curiosity, or surfaced growth pain beyond media allocation.
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <Card>
                  ...
                  <div style={{ width: "100%", height: 320 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={analysis.fragilityPillars}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="name" />
                        <YAxis />
                        <Tooltip />
