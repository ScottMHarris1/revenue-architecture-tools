    ...
    <div className="space-y-8">
      <div className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {[
            ...
            return (
              <div key={item.title} className="rounded-2xl bg-slate-100 p-4">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="h-full rounded-[28px] border-0 shadow-sm">
            <CardHeader>
              <div className="mb-3 flex items-center gap-2">
                <Badge className="rounded-full">Stage 1</Badge>
                <Badge variant="outline" className="rounded-full">Open conversation</Badge>
              </div>
              <CardTitle className="text-2xl">CAC Creep Calculator</CardTitle>
              <CardDescription>
                Use live in client conversations to diagnose whether rising CAC is driven by allocation, pipeline conversion, or both.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl bg-slate-100 p-4 text-sm text-slate-700">Diagnose allocation-led vs pipeline-led pressure</div>
                <div className="rounded-2xl bg-slate-100 p-4 text-sm text-slate-700">Generate a controlled test design</div>
                <div className="rounded-2xl bg-slate-100 p-4 text-sm text-slate-700">Anchor on blended CAC, not just ROAS</div>
                <div className="rounded-2xl bg-slate-100 p-4 text-sm text-slate-700">Equip reps with a stronger talk track</div>
              </div>
              <Button className="w-full rounded-2xl" onClick={() => setScreen("cac")}>Start conversation tool</Button>
            </CardContent>
          </Card>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }}>
          <Card className="h-full rounded-[28px] border-0 shadow-sm">
            <CardHeader>
              <div className="mb-3 flex items-center gap-2">
                <Badge className="rounded-full">Stage 2</Badge>
                <Badge variant="outline" className="rounded-full">Expand diagnosis</Badge>
              </div>
              <CardTitle className="text-2xl">Revenue Fragility Diagnostic</CardTitle>
              <CardDescription>
                Use after traction is established to assess resilience, governance, dependency, and scalability across the client’s revenue system.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl bg-slate-100 p-4 text-sm text-slate-700">Surface structural growth risk</div>
                <div className="rounded-2xl bg-slate-100 p-4 text-sm text-slate-700">Create a strategic escalation path</div>
                <div className="rounded-2xl bg-slate-100 p-4 text-sm text-slate-700">Benchmark durability, not just growth</div>
                <div className="rounded-2xl bg-slate-100 p-4 text-sm text-slate-700">Support deeper advisory follow-up</div>
              </div>
              <Button variant="outline" className="w-full rounded-2xl" onClick={() => setScreen("fragility")}>Run diagnostic</Button>
            </CardContent>
          </Card>
        </motion.div>
      </div>
    ...
    <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        <div className="mb-2 flex items-center gap-2">
          <Badge className="rounded-full px-3 py-1 text-xs">Agency Revenue System Tools</Badge>
          <Badge variant="outline" className="rounded-full px-3 py-1 text-xs">{subtitle}</Badge>
        </div>
        <h1 className="text-3xl font-semibold tracking-tight md:text-5xl">{title}</h1>
      </div>
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" onClick={() => setScreen("home")} className="rounded-2xl">
          <ChevronLeft className="mr-2 h-4 w-4" /> Back to workflow
        </Button>
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-7xl p-4 md:p-8">
        {state.screen === "home" ? (
          ...
          <>
            <TopBar title="CAC Creep Calculator" subtitle="Open conversation" />
            <div className="grid gap-6 lg:grid-cols-[1.05fr_1.35fr]">
              <Card className="rounded-3xl border-0 shadow-sm">
                <CardHeader>
                  <CardTitle>Conversation inputs</CardTitle>
                  <CardDescription>Set the portfolio context and test design in real time with the agency.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-8">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2 md:col-span-2">
                      <Label>Company</Label>
                      <Input value={state.company} onChange={(e) => update("company", e.target.value)} className="rounded-2xl" />
                    </div>
                    <div className="space-y-2">
                      <Label>Benchmark mode</Label>
                      <Select value={state.benchmarkMode} onValueChange={(v) => update("benchmarkMode", v)}>
                        <SelectTrigger className="rounded-2xl"><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="mid-market-agency">Mid-market agency</SelectItem>
                          <SelectItem value="enterprise-agency">Enterprise agency</SelectItem>
                          <SelectItem value="inside-sales-team">Inside sales team</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-2">
                      <Label>Accounts in portfolio</Label>
                      <Input type="number" value={state.accountCount} onChange={(e) => update("accountCount", Number(e.target.value))} className="rounded-2xl" />
                    </div>
                  </div>
                  <Separator />
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2"><Label>Current monthly spend</Label><Input type="number" value={state.monthlySpend} onChange={(e) => update("monthlySpend", Number(e.target.value))} className="rounded-2xl" /></div>
                    <div className="space-y-2"><Label>Current monthly conversions</Label><Input type="number" value={state.monthlyConversions} onChange={(e) => update("monthlyConversions", Number(e.target.value))} className="rounded-2xl" /></div>
                    <div className="space-y-2"><Label>Prior monthly spend</Label><Input type="number" value={state.priorMonthlySpend} onChange={(e) => update("priorMonthlySpend", Number(e.target.value))} className="rounded-2xl" /></div>
                    <div className="space-y-2"><Label>Prior monthly conversions</Label><Input type="number" value={state.priorMonthlyConversions} onChange={(e) => update("priorMonthlyConversions", Number(e.target.value))} className="rounded-2xl" /></div>
                  </div>
                  <Separator />
                  <div className="space-y-6">
                    <div>
                      <div className="mb-2 flex items-center justify-between text-sm"><Label>Demand capture mix</Label><span className="text-slate-500">{state.captureMix}%</span></div>
                      <Slider value={[state.captureMix]} min={0} max={100} step={1} onValueChange={([v]) => { update("captureMix", v); update("discoveryMix", 100 - v); }} />
                    </div>
                    <div>
                      <div className="mb-2 flex items-center justify-between text-sm"><Label>Demand discovery mix</Label><span className="text-slate-500">{state.discoveryMix}%</span></div>
                      <Slider value={[state.discoveryMix]} min={0} max={100} step={1} onValueChange={([v]) => { update("discoveryMix", v); update("captureMix", 100 - v); }} />
                    </div>
                  </div>
                  <Separator />
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2"><Label>Current win rate (%)</Label><Input type="number" value={state.winRate} onChange={(e) => update("winRate", Number(e.target.value))} className="rounded-2xl" /></div>
                    <div className="space-y-2"><Label>Prior win rate (%)</Label><Input type="number" value={state.priorWinRate} onChange={(e) => update("priorWinRate", Number(e.target.value))} className="rounded-2xl" /></div>
                    <div className="space-y-2"><Label>Current avg sales cycle (days)</Label><Input type="number" value={state.avgSalesCycleDays} onChange={(e) => update("avgSalesCycleDays", Number(e.target.value))} className="rounded-2xl" /></div>
                    <div className="space-y-2"><Label>Prior avg sales cycle (days)</Label><Input type="number" value={state.priorSalesCycleDays} onChange={(e) => update("priorSalesCycleDays", Number(e.target.value))} className="rounded-2xl" /></div>
                    <div className="space-y-2 md:col-span-2"><Label>Deals above stage-aging threshold (%)</Label><Input type="number" value={state.stageAgingOverThreshold} onChange={(e) => update("stageAgingOverThreshold", Number(e.target.value))} className="rounded-2xl" /></div>
                  </div>
                  <Separator />
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2"><Label>Test reallocation (%)</Label><Input type="number" value={state.testReallocation} onChange={(e) => update("testReallocation", Number(e.target.value))} className="rounded-2xl" /></div>
                    <div className="space-y-2"><Label>Readout window (weeks)</Label><Input type="number" value={state.targetReadoutWeeks} onChange={(e) => update("targetReadoutWeeks", Number(e.target.value))} className="rounded-2xl" /></div>
                  </div>
                </CardContent>
              </Card>
                      ...
                      <motion.div key={card.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }}>
                        <Card className="rounded-3xl border-0 shadow-sm">
                          <CardContent className="p-5">
                            <div className="mb-4 flex items-center justify-between"><div className="rounded-2xl bg-slate-100 p-2"><Icon className="h-4 w-4" /></div><Badge variant="outline" className="rounded-full">{state.company}</Badge></div>
                            <div className="text-sm text-slate-500">{card.title}</div>
                            <div className="mt-2 text-2xl font-semibold leading-tight">{card.value}</div>
                ...
                <Card className="rounded-3xl border-0 shadow-sm">
                  <CardHeader>
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div><CardTitle>Rep summary</CardTitle><CardDescription>Use this live to reframe the conversation and propose a next step.</CardDescription></div>
                      <Badge className="rounded-full px-4 py-1 text-sm">{analysis.diagnosis}</Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <div className="grid gap-6 xl:grid-cols-[1.25fr_0.95fr]">
                      <div>
                        <p className="text-base leading-7 text-slate-700">{analysis.narrative}</p>
                        <div className="mt-6 grid gap-4 md:grid-cols-3">
                          ...
                          <div className="rounded-2xl bg-slate-100 p-4"><div className="text-xs uppercase tracking-wide text-slate-500">Modeled CAC improvement</div><div className="mt-2 text-xl font-semibold">{formatPct(analysis.modeledCACImprovementPct)}</div><div className="mt-1 text-sm text-slate-600">Hold-flat spend test</div></div>
                          <div className="rounded-2xl bg-slate-100 p-4"><div className="text-xs uppercase tracking-wide text-slate-500">Incremental conversions</div><div className="mt-2 text-xl font-semibold">{Math.round(analysis.incrementalConversions)}</div><div className="mt-1 text-sm text-slate-600">Modeled from new blended CAC</div></div>
                        </div>
